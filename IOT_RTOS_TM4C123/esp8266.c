...
void connect_esp_fetch_data_test(){
  printf("\n\r-----------\n\rTrying to connect to wifi...\n\r");
...
char string_3[10];
char string_4[10];
char data2Header[]="%20%25&data2=";
char data3Header[]="%20&data3=";
char endOfData[]="&edxcode=8086 HTTP/1.1\r\nUser-Agent: Keil\r\nHost: emb445l.appspot.com\r\n\r\n";

uint32_t logOwnServer(uint32_t data1,uint32_t data2, uint32_t data3){
  sprintf(data1_buff,"%d",data1);
  sprintf(data2_buff,"%d",data2);
  sprintf(data3_buff,"%d",data3);

  strcpy(string_1,"GET /query?city=Austin%20Texas&id=Wei%20and%20Corentin&data1=");
    if(data1_buff[1]!=NULL){
 ...
 // string_3[2]=data2_buff[2];
  ...
  string_4[2]=data3_buff[2];
  string_4[3]=data3_buff[3];
  }
  else if(data3_buff[2]!=NULL){
  string_4[0]=data3_buff[0];
  string_4[1]=data3_buff[1];
  string_4[2]=data3_buff[2];
  }
  else if(data3_buff[1]!=NULL){
  string_4[0]=data3_buff[0];
  string_4[1]=data3_buff[1];
  }
  else{
