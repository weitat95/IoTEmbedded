...
int32_t ChannelNumber = -1;             // Which ADC channel is open. -1 implies closed
int32_t NumberSamples = 0;              // Number of samples requested
int32_t TimerChannelNumber = -1;    // Which channel is being used by the timer / Closed{-1)
uint16_t* BufferName = (void *) 0;                      // Buffer in which we store ADC values
...
void ADC0_InitSWTriggerSeq2(uint8_t channelNum){ 
  volatile uint32_t delay;
  switch(channelNum){             // 1) activate clock
    case 0:
    case 1:
...
void ADC_Init(uint8_t channelNum) {
    ...
    ADC0_InitSWTriggerSeq2(channelNum);
