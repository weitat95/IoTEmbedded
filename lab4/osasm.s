...
;// Creation: 1/23/2017
;// Description: Provide assembly functions for the OS
;// Lab 2
...
PendSV_Handler						; 1) Saves R0-R3,R12,LR,PC,PSR
	...
	STR		SP,[R1]					; 5) Save SP into TCB
	; Scheduler
	LDR		R1, =NextPt             ; 6) R1 = pointer to NextPr, new thread
	LDR		R2,[R1]					;    R2 = value of NextPtr (next TCB address)
	STR		R2,[R0] 				;    RunPt = R2                	; Fix this
	; Restore registers
	LDR		SP,[R2] 				; 7) new thread SP; SP = RunPt->sp;
	POP		{R4-R11}				; 8) restore regs r4-11
	CPSIE	I						; 9) tasks run with interrupts enabled
	BX		LR						; 10) restore R0-R3,R12,LR,PC,PSR
	
;NOT USED (Just Spin Lock Semaphore)
...
OS_Signal	; RO points to counter
	LDREX	R1, [R0]	; counter
	ADD		R1, #1		; counter + 1
