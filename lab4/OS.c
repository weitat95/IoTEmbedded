...
void Timer5A_Handler(void){
  ...
  if(flag2){
      ...
      if(diff>period2){
        jitter2 = (diff-period2+4)/8;  // in 0.1 usec
      }else{
        jitter2 = (period2-diff+4)/8;  // in 0.1 usec
      }
      if(jitter2>100){
        jitter2=0;
      }
      if(jitter2 > MaxJitter2){
...
void Jitter(void){
  //Uint32_t MaxJitter1_=MaxJitter1;
  //Uint32_t MaxJitter2_=MaxJitter2;
