// Name: ST7735.c
// Authors: Daniel Valvano, modified by Corentin Dugue & Wei Tat Lee
// Creation: 03/30/2015
// Description: Provide functions for using ST7735 display
// Lab 1
// TA: Kishore Punniyamurthy
// Last modified: 02/06/2017
// Hardware connections: see below
...
void Delay1ms(uint32_t n){uint32_t volatile time;
  while(n){
    //time = 727240*2/91;  // 1msec from old file
...
/**************ST7735_Line**************
 Draw a line between two points defined
 Inputs:  x1   x point origin
          y1   y point origin
...
// *************** ST7735_Message2 ********************
// Output one character to one of the two logically separate displays
// Display selected by int device
// Line selected by int line
// Inputs:    device int to select display
//            line int to select line
//            value int32_t number to display
// Outputs: none
void STT7735_Message2(int device, int line, int dist, int row, int col, char *string, int32_t value) {
  ...
  // Decide which color to print
  switch(device) {
    case 1: TxtColor = ST7735_YELLOW; break;
    case 2: TxtColor = ST7735_CYAN; break;
    case 3: TxtColor = ST7735_RED; break;
    case 4: TxtColor = ST7735_BLUE; break;
    case 5: TxtColor = ST7735_GREEN; break;
    case 6: TxtColor = ST7735_MAGENTA; break;
    default: TxtColor = ST7735_WHITE;
  }

  // Compute distance between rows/columns
  uint16_t rowInt = 159/row;
  uint16_t colInt = 128/col;

  // Draw lines
  for(j=1; j<row; j++) {
  ST7735_Line(0, rowInt*j, 127, rowInt*j, ST7735_WHITE);
  }
  for(k=1; k<col; k++) {
  ST7735_Line(k*colInt, 0, k*colInt, 159, ST7735_WHITE);
  }

  // Print string
  while(*string){
    ST7735_DrawCharS(x*6+dist*colInt+2, line*10+rowInt*device+1, *string, TxtColor, ST7735_BLACK, 1);
    string++;
    x++;
    count++;
  }
  ...
  // Print value if different from 0
 // if(value !=0) {
    Messageindex = 0;
    fillmessage(value);
    uint16_t i=0;
    for(i=0; i<Messageindex; i++){
      ST7735_DrawCharS(x*6+dist*colInt+2, line*10+rowInt*device+1, Message[i], TxtColor, ST7735_BLACK, 1);
      x++;
    }

    for(i=0;i<4-Messageindex;i++){
      ST7735_DrawCharS(x*6+dist*colInt+2, line*10+rowInt*device+1, ' ', TxtColor, ST7735_BLACK, 1);
        x++;
    }
  //}
...
// Inputs:    device int to select display
//            line int to select line
//            dist int to select which column to print the string/value in
//            row int to define number of rows
//            col int to define number of columns
//            char *string point null terminated ASCII string
//            value int32_t number to display
// Outputs: none
