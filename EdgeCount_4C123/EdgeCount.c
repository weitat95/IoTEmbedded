...
// external signal connected to PB6 (T0CCP0) (count on falling edge)
...
void EdgeCount_Init(void){
  ...
  TIMER0_CTL_R |= 0xC;      // configure for both rising and falling edge
  TIMER0_TAILR_R = 0xFFFF;
  TIMER0_TAPR_R = 0xFF;        // counts up to 0xFFFFFF (16,777,215)
  TIMER0_TAMATCHR_R = 0x10;
  TIMER0_TAPMR_R = 0x10;
...
void (*ProducerTask)(int,unsigned long);
void EdgeTimer_Init(void(*task)(int,unsigned long)){
  SYSCTL_RCGCTIMER_R |= 0x01;  // activate Timer 0
  SYSCTL_RCGCGPIO_R |= 0x02;   // activate Port B
                               // allow time to finish activating
  while((SYSCTL_PRGPIO_R&0x0002) == 0){};
  ...
  GPIO_PORTB_AMSEL_R &= ~0x40; // disable analog functionality on PB6
	...
	TIMER0_CTL_R &= ~0x00000001; // disable timer0A during setup
  TIMER0_CFG_R = 0x00000004;   // configure for 16-bit edge timer mode
	TIMER0_TAMR_R = 0x007;				//Count Up
	TIMER0_CTL_R |= 0xC;      // configure for both rising and falling edge
...
void Timer0A_Handler(void){
	TIMER0_ICR_R = 0x00000004;// acknowledge timer0A event timeout
	counter++;
	...
	TIMER0_TAILR_R=0xFFFF;
	TIMER0_TAPR_R=0xFF;
  (*ProducerTask)(edge,timer);                // execute user task
}
