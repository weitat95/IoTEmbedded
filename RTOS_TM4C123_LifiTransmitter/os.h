...
struct  Sema4{
  long Value;   
// >0 means free, otherwise means busy        
...
//******** OS_AddProcess *************** 
// Add one process into the system, creating a new PCB block
// Won't create more than NUMPROCESS
// Inputs:     entry : main of the new process
//              text : starting address of code/text
//              data : starting address of data
//         stackSize : Stack size (not used)
//          priority : Priority of the Process(RoundRobin not used)
// Outputs: Successful(1) or Fail(0)
int OS_AddProcess(void(*entry)(void), void *text, void *data, 
                  unsigned long stackSize, unsigned long priority);

void OS_DelayMS(unsigned long milliseconds);

void OS_StartCritical(void);
