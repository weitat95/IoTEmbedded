...
#include "edisk.h"		/* Declarations of low level disk I/O functions */
#include "eDisk.h"
/*--------------------------------------------------------------------------
...
)					/* Move to zero only writes back dirty window */
{
	...
	if (wsect != sector) {	/* Changed current window */
		...
		if (fs->wflag) {	/* Write back dirty window if needed */
			if (eDisk_Write(fs->drv, fs->win, wsect, 1) != RES_OK)
				return FR_DISK_ERR;
			fs->wflag = 0;
			if (wsect < (fs->fatbase + fs->fsize)) {	/* In FAT area */
				...
				for (nf = fs->n_fats; nf > 1; nf--) {	/* Reflect the change to all FAT copies */
					wsect += fs->fsize;
					eDisk_Write(fs->drv, fs->win, wsect, 1);
				}
			}
		}
#endif
		if (sector) {
			if (eDisk_Read(fs->drv, fs->win, sector, 1) != RES_OK)
				return FR_DISK_ERR;
			fs->winsect = sector;
		}
	}
...
)
{
	...
	if (res == FR_OK) {
		...
		if (fs->fs_type == FS_FAT32 && fs->fsi_flag) {
			...
			eDisk_Write(fs->drv, fs->win, fs->fsi_sector, 1);
			fs->fsi_flag = 0;
		}
...
{
	if (eDisk_Read(fs->drv, fs->win, sect, 1) != RES_OK)	/* Load boot record */
		return 3;
...
)
{
	...
	*rfs = fs = FatFs[vol];				/* Return pointer to the corresponding file system object */
	...
	stat = eDisk_Init(fs->drv);	/* Initialize low level disk I/O layer */
	...
	if (fmt == FS_FAT32) {
		...
		if (eDisk_Read(fs->drv, fs->win, fs->fsi_sector, 1) == RES_OK &&
...
)
{
	...
	if (eDisk_Status(fs->drv) & STA_NOINIT)
		return FR_NOT_READY;
	...
	for ( ;  btr;								/* Repeat until all data read */
		...
		if ((fp->fptr % SS(fp->fs)) == 0) {		/* On the sector boundary? */
			...
			if (cc) {							/* Read maximum contiguous sectors directly */
				...
				if (eDisk_Read(fp->fs->drv, rbuff, sect, (BYTE)cc) != RES_OK)
			...
			if (fp->dsect != sect) {			/* Load data sector if not in cache */
				...
				if (fp->flag & FA__DIRTY) {		/* Write-back dirty sector cache */
					if (eDisk_Write(fp->fs->drv, fp->buf, fp->dsect, 1) != RES_OK)
						ABORT(fp->fs, FR_DISK_ERR);
					fp->flag &= ~FA__DIRTY;
				}
#endif
				if (eDisk_Read(fp->fs->drv, fp->buf, sect, 1) != RES_OK)	/* Fill sector cache */
					ABORT(fp->fs, FR_DISK_ERR);
			}
#endif
			fp->dsect = sect;
	...
	for ( ;  btw;							/* Repeat until all data written */
		...
		if ((fp->fptr % SS(fp->fs)) == 0) {	/* On the sector boundary? */
			...
			if (cc) {						/* Write maximum contiguous sectors directly */
				if (csect + cc > fp->fs->csize)	/* Clip at cluster boundary */
					cc = fp->fs->csize - csect;
				if (eDisk_Write(fp->fs->drv, wbuff, sect, (BYTE)cc) != RES_OK)
					ABORT(fp->fs, FR_DISK_ERR);
#if _FS_TINY
			...
			if (fp->dsect != sect) {		/* Fill sector cache with file data */
				if (fp->fptr < fp->fsize &&
					eDisk_Read(fp->fs->drv, fp->buf, sect, 1) != RES_OK)
						ABORT(fp->fs, FR_DISK_ERR);
			}
#endif
			fp->dsect = sect;
...
#endif
					if (eDisk_Read(fp->fs->drv, fp->buf, dsc, 1) != RES_OK)	/* Load current sector */
						ABORT(fp->fs, FR_DISK_ERR);
#endif
					fp->dsect = dsc;
				}
...
#endif
			if (eDisk_Read(fp->fs->drv, fp->buf, nsect, 1) != RES_OK)	/* Fill sector cache */
				ABORT(fp->fs, FR_DISK_ERR);
#endif
			fp->dsect = nsect;
		}
...
)
{
	...
	if (sfd) {	/* No patition table (SFD) */
		...
		if (eDisk_Write(drv, fs->win, 0, 1) != RES_OK)	/* Put the MBR into first physical sector */
			return FR_DISK_ERR;
		md = 0xF8;
	}
	...
	if (eDisk_Write(drv, tbl, b_vol, 1) != RES_OK)	/* Write VBR */
		return FR_DISK_ERR;
	if (fmt == FS_FAT32)							/* Write backup VBR if needed (VBR+6) */
		eDisk_Write(drv, tbl, b_vol + 6, 1);
	...
	for (i = 0; i < N_FATS; i++) {		/* Initialize each FAT copy */
		...
		if (eDisk_Write(drv, tbl, wsect++, 1) != RES_OK)
			return FR_DISK_ERR;
	...
	if (fmt == FS_FAT32) {
		...
		eDisk_Write(drv, tbl, b_vol + 1, 1);	/* Write original (VBR+1) */
		eDisk_Write(drv, tbl, b_vol + 7, 1);	/* Write backup (VBR+7) */
	}
